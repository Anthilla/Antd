...
namespace antdlib.views {
    [Serializable]
    public class NftModel : EntityModel {
        public NftModel() {
        ...
        public NftModel(NftSchema sourceModel) {
    ...
    #region [    View    ]
    public class NftSchema : RDBSchema {
        //---
    ...
    [RegisterView]
    public class NftView : View<NftModel> {
        public NftView() {
            Name = "Nft";
            Description = "Primary view for NftModel";
            isPrimaryList = true;
            isActive = true;
            BackgroundIndexing = false;
            ConsistentSaveToThisView = true;
            Version = 8;
            Schema = typeof(NftSchema);
            Mapper = (api, docid, doc) => {
                if (doc.Status != EntityStatus.New) return;
                var k = doc.Key.ToKey();
                var v = doc.Vector.ToVector();
                var decryptedDoc = Encryption.DbDecrypt<NftModel>(doc.Dump, k, v);
                doc = decryptedDoc;
                object[] schemaNfts = {
                    doc.Id.ToString(),
                    doc.Guid,
