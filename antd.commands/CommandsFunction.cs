...
namespace antd.commands {
    public class CommandsFunction {
        ...
        private static readonly Read Read = new Read();


        private static Dictionary<string, CommandsOptions> GetDict2() {
            var dict = new Dictionary<string, CommandsOptions>();
            dict["bond-add-if"] = new CommandsOptions {
                Commands = new[] {
                    "ifenslave $bond $net_if"
                },
                Function = _ => Bash.Execute("")
            };
            return dict;
        }


      //  private static Dictionary<string, Func<dynamic>> GetDict() {
      //      var dict = new Dictionary<string, Func<dynamic>> {
      //          {"anthilla",
      //              Bash.Execute( new [] {"$custom"} )
      //          },
      //          {
      //          "bond-add-if", new[] {
      //              "ifenslave $bond $net_if"
