...
namespace NWebDav.Server.HttpListener {
    public class HttpBasicContext : HttpBaseContext {
        public HttpBasicContext(HttpListenerContext httpListenerContext, Func<HttpListenerBasicIdentity, IPrincipal> getPrincipal)
        ...
        public HttpBasicContext(HttpListenerContext httpListenerContext, Func<HttpListenerBasicIdentity, bool> checkIdentity)
            : base(httpListenerContext.Request, httpListenerContext.Response) {
            // Obtain the basic identity
            var basicIdentity = httpListenerContext.User?.Identity as HttpListenerBasicIdentity;
