...
namespace Kvpbase {
    public class Node {
        ...
        public static Node DetermineOwner(string userGuid, UserManager users, Topology topology, Node currentNode) {
            #region Check-for-Null-Values

            if(String.IsNullOrEmpty(userGuid)) {
                ConsoleLogger.Warn("DetermineOwner null user GUID supplied");
                return null;
            }
            ...
            if((topology == null)
                ...
                ConsoleLogger.Warn("DetermineOwner null topology or no nodes in topology");
                return null;
            }
            ...
            if(currUser != null) {
                if(currUser.NodeId > 0) {
                    ...
                    if(currUser.NodeId == currentNode.NodeId) {
                        ConsoleLogger.Warn("DetermineOwner GUID " + userGuid + " statically mapped to self (NodeId " + currentNode.NodeId + ")");
                        return currentNode;
                    }

                    foreach(Node curr in topology.Nodes) {
                        if(curr.NodeId == currUser.NodeId) {
                            ConsoleLogger.Warn("DetermineOwner GUID " + userGuid + " statically mapped to NodeId " + curr.NodeId);
                            return curr;
                        }
                    }
            ...
            foreach(Node curr in sorted_list) {
                if(currPos == matchPos) {
                    ConsoleLogger.Warn("DetermineOwner primary for user GUID " + userGuid + " is " + curr.Name + " (" + curr.DnsHostname + ":" + curr.Port + ":" + curr.Ssl + ")");
                    return curr;
                }
            ...
            ConsoleLogger.Warn("DetermineOwner iterated all nodes in sorted list, did not encounter " + matchPos + " entries");
            return null;
