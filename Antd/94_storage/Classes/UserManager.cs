...
namespace Kvpbase {
    public class UserManager {
        ...
        public UserMaster GetUserByEmail(string email) {
            ...
            lock(UserLock) {
                foreach(UserMaster curr in Users) {
                    if(String.Compare(curr.Email, email) == 0)
                        return curr;
                }
            }
            return null;
        ...
        public UserMaster GetUserById(int? id) {
            ...
            lock(UserLock) {
                foreach(UserMaster curr in Users) {
                    if(curr.UserMasterId == idInternal)
                        return curr;
                }
            }
            return null;
        ...
        public bool AuthenticateCredentials(string email, string password, out UserMaster curr) {
            ...
            if(curr == null) {
                ConsoleLogger.Warn("AuthenticateCredentials unable to find email " + email);
                return false;
            }
            ...
            if(String.Compare(curr.Password, password) == 0) {
                if(curr.Active == 1) {
                    if(Common.IsLaterThanNow(curr.Expiration)) {
                        ...
                        ConsoleLogger.Warn("AuthenticateCredentials UserMasterId " + curr.UserMasterId + " expired at " + curr.Expiration);
                        return false;
                    }
                }
                else {
                    ConsoleLogger.Warn("AuthenticateCredentials UserMasterId " + curr.UserMasterId + " marked inactive");
                    return false;
                }
            }
            else {
                ConsoleLogger.Warn("AuthenticateCredentials invalid password supplied for email " + email + " (" + password + " vs " + curr.Password + ")");
                return false;
            }
        }
        ...
        //todo qui posso cambiare la gestione delle cartelle a cui gli utenti possono accederes
        public string GetHomeDirectory(string guid, Settings settings) {
            ...
            UserMaster curr = GetUserByGuid(guid);
            if(curr == null) {
                ConsoleLogger.Warn("GetHomeDirectory unable to find user GUID " + guid);
                return null;
            }
