...
namespace Kvpbase {
    public static class Common {
        ...
        public static void ExitApplication(string method, string text, int returnCode) {
            ConsoleLogger.Log($"[Storage Server] exit: {text}");
            //ConsoleLogger.Log("The server has stop working.");
            //ConsoleLogger.Log("");
            //ConsoleLogger.Log("  Requested by : " + method);
            //ConsoleLogger.Log("  Reason text  : " + text);
            //ConsoleLogger.Log("");
            //ConsoleLogger.Log("---");
            return;
        }
        ...
        public static int InputInteger(string question, int defaultAnswer, bool positiveOnly, bool allowZero) {
            while(true) {
                ...
                if(!Int32.TryParse(userInput, out ret)) {
                    ConsoleLogger.Log("Please enter a valid integer.");
                    continue;
                }
                ...
                if(ret < 0) {
                    if(positiveOnly) {
                        ConsoleLogger.Log("Please enter a value greater than zero.");
                        continue;
                    }
                }
        ...
        public static bool VerifyDirectoryAccess(string environment, string directory) {
            ...
            switch(environment) {
                case "linux":
                    ...
                    //if(!ufi.CanAccess(AccessModes.F_OK))
                    //    return false;
                    //if(!ufi.CanAccess(AccessModes.R_OK))
                    //    return false;
                    //if(!ufi.CanAccess(AccessModes.W_OK))
                    //    return false;
                    //if(!ufi.CanAccess(AccessModes.X_OK))
                    //    return false;
                    return true;
        ...
        public static List<string> GetFileList(string environment, string directory, bool prependFilename) {
            try {
                ...
                foreach(FileInfo file in files) {
                    if(prependFilename)
                        fileList.Add(directory + separator + file.Name);
                    else
        ...
        public static bool ContainsUnsafeCharacters(List<string> data) {
            ...
            foreach(string curr in data) {
                if(ContainsUnsafeCharacters(curr))
                    return true;
            }
            return false;
        }
        ...
        public static T DeserializeJsonBuiltIn<T>(string json) {
            ...
            try {
                ...
                ConsoleLogger.Log("");
                ConsoleLogger.Log("Exception while deserializing:");
                ConsoleLogger.Log(json);
                ConsoleLogger.Log("");
                throw e;
            }
        }
