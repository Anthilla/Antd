...
namespace Kvpbase {
    public class ApiKeyManager {
            ...
            if(apiKeyId == null) {
                ...
                if(perms != null && perms.Count > 0) {
                    foreach(ApiKeyPermission curr in perms) {
                        if(!Common.IsTrue(curr.Active))
                            continue;
                        if(!Common.IsLaterThanNow(curr.Expiration))
                            continue;

                        if(Common.IsTrue(curr.AllowReadObject))
                            ret.AllowReadObject = 1;
                        if(Common.IsTrue(curr.AllowReadContainer))
                            ret.AllowReadContainer = 1;
                        if(Common.IsTrue(curr.AllowWriteObject))
                            ret.AllowWriteObject = 1;
                        if(Common.IsTrue(curr.AllowWriteContainer))
                            ret.AllowWriteContainer = 1;
                        if(Common.IsTrue(curr.AllowDeleteObject))
                            ret.AllowDeleteObject = 1;
                        if(Common.IsTrue(curr.AllowDeleteContainer))
                            ret.AllowDeleteContainer = 1;
                        if(Common.IsTrue(curr.AllowSearch))
                            ret.AllowSearch = 1;
                    }
                }
        ...
        public bool VerifyApiKey(
            ...
            if(currApiKey == null) {
                ConsoleLogger.Warn("VerifyApiKey unable to retrieve API key " + apiKey);
                return false;
            }

            currPermission = GetEffectiveApiKeyPermissions(currApiKey.ApiKeyId, currUserMaster.UserMasterId);
            if(currPermission == null) {
                ConsoleLogger.Warn("VerifyApiKey unable to build ApiKeyPermission object for UserMasterId " + currUserMaster.UserMasterId);
                return false;
            }
            ...
            if(currApiKey.Active == 1) {
                ...
                if(Common.IsLaterThanNow(currApiKey.Expiration)) {
                    ...
                    if(currUserMaster == null) {
                        ConsoleLogger.Warn("VerifyApiKey unable to find UserMasterId " + currApiKey.UserMasterId);
                        return false;
                    }
                    ...
                    if(currUserMaster.Active == 1) {
                        ...
                        if(Common.IsLaterThanNow(currUserMaster.Expiration)) {
                            ...
                            ConsoleLogger.Warn("VerifyApiKey UserMasterId " + currUserMaster.UserMasterId + " expired at " + currUserMaster.Expiration);
                            return false;
                        }
                        ...
                        ConsoleLogger.Warn("VerifyApiKey UserMasterId " + currUserMaster.UserMasterId + " marked inactive");
                        return false;
                    }
                }
                else {
                    ConsoleLogger.Warn("VerifyApiKey ApiKeyId " + currApiKey.ApiKeyId + " expired at " + currApiKey.Expiration);
                    return false;
                }
                ...
                ConsoleLogger.Warn("VerifyApiKey ApiKeyId " + currApiKey.ApiKeyId + " marked inactive");
                return false;
            }
        }
