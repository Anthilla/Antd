...
namespace Kvpbase {
    public class FailedRequestsThread {
        public FailedRequestsThread(Settings settings, ConcurrentQueue<Dictionary<string, object>> queue) {
            if(settings == null)
                throw new ArgumentNullException(nameof(settings));
            if(queue == null)
                queue = new ConcurrentQueue<Dictionary<string, object>>();
            Task.Run(() => FailedRequestsWorker(settings, queue));
        }

        private void FailedRequestsWorker(Settings settings, ConcurrentQueue<Dictionary<string, object>> queue) {
            #region Setup
            ...
            if(settings.Server.FailedRequestsIntervalSec < 60) {
                ConsoleLogger.Warn("FailedRequestsWorker setting failed requests interval 60 sec (config value too low: " + settings.Server.FailedRequestsIntervalSec + " sec)");
                settings.Server.FailedRequestsIntervalSec = 60;
            }

            ConsoleLogger.Warn("FailedRequestsWorker starting with failed requests interval set to " + settings.Server.FailedRequestsIntervalSec + " sec");
        ...
        private string EmailBody(Settings settings, List<Dictionary<string, object>> dicts) {
            ...
            if(dicts == null || dicts.Count < 1) {
                ...
                foreach(Dictionary<string, object> curr in dicts) {
                    ...
                    if(curr.ContainsKey("status"))
                        para += "<li>Status code: " + curr["status"].ToString() + "</li>";
