    ...
    public class Message {
        ...
        public static void SendMessage(
            ...
            if(resp == null) {
                ...
                ConsoleLogger.Warn("SendMessage null response connecting to " + url + ", message will be queued");
                success = false;
                ...
                if(resp.StatusCode != 200) {
                    ...
                    ConsoleLogger.Warn("SendMessage non-200 response connecting to " + url + ", message will be queued");
                    success = false;
            ...
            if(!success) {
                ...
                if(!Common.DirectoryExists(settings.Messages.Directory + to.Name)) {
                    try {
                        ...
                        ConsoleLogger.Warn("SendMessage exception while creating directory " + settings.Messages.Directory + to.Name);
                        ConsoleLogger.Error("SendMessage Exception while creating directory " + settings.Messages.Directory + to.Name + e.ToString());
                        return;
                    }
                }
                ...
                while(true) {
                    ...
                    if(loopCount > 16) {
                        ConsoleLogger.Warn("SendMessage unable to generate unused GUID for folder " + settings.Messages.Directory + to.Name + ", exiting");
                        return;
                    }
                }
                ...
                if(!Common.WriteFile(
                    ...
                    false)) {
                    ConsoleLogger.Warn("SendMessage unable to write message to " + settings.Messages.Directory + to.Name + Common.GetPathSeparator(settings.Environment) + guid + ", exiting");
                    return;
                }

                ConsoleLogger.Warn("SendMessage queued message to " + settings.Messages.Directory + to.Name + Common.GetPathSeparator(settings.Environment) + guid);
