            ...
            try {
                ...
                if(!String.IsNullOrEmpty(readFromVal)) {
                    if(!Int32.TryParse(readFromVal, out readFrom)) {
                        ConsoleLogger.Warn("GetPublicFile invalid value for read_from in querystring: " + readFromVal);
                    ...
                    if(readFrom < 0) {
                        ConsoleLogger.Warn("GetPublicFile invalid value for read_from (must be zero or greater): " + readFrom);
                        return new HttpResponse(md.CurrentHttpRequest, false, 400, null, "application/json",
                            new ErrorResponse(2, 400, "Invalid value for read_from.", null).ToJson(), true);
                    }
                }
                ...
                if(!String.IsNullOrEmpty(countVal)) {
                    if(!Int32.TryParse(countVal, out count)) {
                        ConsoleLogger.Warn("GetPublicFile invalid value for count in querystring: " + countVal);
                        return new HttpResponse(md.CurrentHttpRequest, false, 400, null, "application/json",
                            new ErrorResponse(2, 400, "Invalid value for count.", null).ToJson(), true);
                    }

                    if(count < 1) {
                        ConsoleLogger.Warn("GetPublicFile invalid value for count (must be greater than zero): " + count);
                        return new HttpResponse(md.CurrentHttpRequest, false, 400, null, "application/json",
                            new ErrorResponse(2, 400, "Invalid value for count.", null).ToJson(), true);
                    }
                }
                ...
                ConsoleLogger.Warn("GetPublicFile URL " + md.CurrentHttpRequest.RawUrlWithoutQuery + " (pubfile GUID " + pubfileGuid + ")");

                if(String.IsNullOrEmpty(pubfileGuid)) {
                    ConsoleLogger.Warn("GetPublicFile null GUID after removing base URL");
                    return new HttpResponse(md.CurrentHttpRequest, false, 400, null, "application/json",
                ...
                if(pubfileGuid.Contains("/")
                    ...
                    ConsoleLogger.Warn("GetPublicFile URL contains invalid characters: " + pubfileGuid);
                    return new HttpResponse(md.CurrentHttpRequest, false, 400, null, "application/json",
                ...
                if(String.IsNullOrEmpty(pubfileContents)) {
                    ConsoleLogger.Warn("GetPublicFile unable to read contents of pubfile " + CurrentSettings.PublicObj.Directory + " (null)");
                ...
                try {
                    ...
                    ConsoleLogger.Warn("GetPublicFile unable to deserialize pubfile at " + CurrentSettings.PublicObj.Directory + pubfileGuid);
                ...
                if(DateTime.Compare(DateTime.Now.ToUniversalTime(), currPubfile.Expiration) > 0) {
                    ConsoleLogger.Warn("GetPublicFile pubfile " + CurrentSettings.PublicObj.Directory + pubfileGuid + " expired, deleting (expired " + currPubfile.Expiration.ToString("MM/dd/yyyy HH:mm:ss"));

                    if(!Common.DeleteFile(CurrentSettings.PublicObj.Directory + pubfileGuid)) {
                        ConsoleLogger.Warn("GetPublicFile unable to delete pubfile " + CurrentSettings.PublicObj.Directory + pubfileGuid);
                    }
                ...
                currObj = Obj.BuildObjFromDisk(currPubfile.DiskPath, Users, CurrentSettings, CurrentTopology, CurrentNode);
                if(currObj == null) {
                    ConsoleLogger.Warn("GetPublicFile unable to overwrite metadata object using disk path " + currPubfile.DiskPath);
                    return new HttpResponse(md.CurrentHttpRequest, false, 500, null, "application/json",
                        new ErrorResponse(1, 500, null, null).ToJson(), true);
                }
                ...
                if(!locked) {
                    ConsoleLogger.Warn("GetPublicFile " + md.CurrentObj.DiskPath + " is unable to be locked");
                ...
                if(Common.IsTrue(md.CurrentObj.IsEncrypted)) {
                    if(String.IsNullOrEmpty(md.CurrentObj.EncryptionKsn)) {
                        ...
                        if(!EncryptionManager.ServerDecrypt(md.CurrentObj.Value, md.CurrentObj.EncryptionKsn, out clear)) {
                            ConsoleLogger.Warn("GetPublicFile unable to decrypt object using server-based decryption");
                            return new HttpResponse(md.CurrentHttpRequest, false, 500, null, "application/json",
                ...
                if(count > 0) {
                    if(readFrom + count > md.CurrentObj.Value.Length) {
                        ConsoleLogger.Warn("GetPublicFile range exceeds object length (" + md.CurrentObj.Value.Length + "): read_from " + readFrom + " count " + count);
                        return new HttpResponse(md.CurrentHttpRequest, false, 400, null, "application/json",
                ...
                if(locked) {
                    if(!LockManager.RemoveReadResource(md.CurrentObj.DiskPath)) {
                        ConsoleLogger.Warn("GetPublicFile unable to unlock " + md.CurrentObj.DiskPath);
                    }
                }
