...
namespace Antd {
    ...
    public class SetAnthillaAS {
        ...
        private static Anth_ParamWriter config = new Anth_ParamWriter(paths, "antdAnthillaAS");

        private static string[] baseScriptArgs = new string[] {
                "qemu-nbd --connect=/dev/nbd2 /System/anthillaas.qed",
                "kpartx -a /dev/nbd2",
                "sleep 1",
                "mkdir -p  /framework/Anthilla/AnthillaAS/version",
                "mount -o rw,noatime,discard /dev/mapper/nbd2p1 /framework/Anthilla/AnthillaAS/version"
            };
