...
var CalHeatMap = function () {
    ...
    this.paint = function (navigationDir) {
        ...
        var subDomainSvgGroup = svg.append("svg")
            ...
            .attr("y", function () {
                if (options.label.position === "top") {
                    return self.domainVerticalLabelHeight + options.domainMargin[0] - 30; //edit: aggiunto - 30
                } else {
                    return options.domainMargin[0] - 30; //edit: aggiunto - 30
                }
            })
        ...
        rect
            ...
            .text(function (d) { return new Date(d.t); })
            .attr("width", options.cellSize - 2)
            .attr("height", options.cellSize - 2)
            .attr("x", function (d) { return self.positionSubDomainX(d.t); }) //edit: aggiunto - 3
            .attr("y", function (d) { return self.positionSubDomainY(d.t); }) //edit: aggiunto - 3
            .on("click", function (d) {
                $(".graph-domain rect[style]").each(function () {
                    $(this).removeAttr("style");
                });
                $(this).attr("style", "stroke: #e74c3c; stroke-width:2;");
        ...
        getDisplayCurrentDateSelected();
        function getDisplayCurrentDateSelected() {
            if ($("#cal-heatmap-button-click").text() != "Find Clicked") {
                $(".graph-domain rect").each(function () {
                    var rectDate = $(this).text();
                    var currentDate = new Date();
                    if (moment(rectDate).format('YYYY/MM/DD') == moment(currentDate).format('YYYY/MM/DD')) {
                        $(this).attr("style", "stroke: #e74c3c; stroke-width:2;");
                    }
                });
            }
        ...
        if (options.domainLabelFormat !== "") {
            svg.append("text")
                ...
                .text(function (d) {
                    ...
                    var m = self.formatDate(new Date(d), options.domainLabelFormat);
                    //var dat = "{{ '" + m + "' | translate }}";
                    //console.log(dat);
                    return m;
                })
        ...
        rect.transition().duration(options.animationDuration).select("text")
            ...
            .attr("class", function (d) {
                if (parent.legendScale === null || (d.v === null && (options.hasOwnProperty("considerMissingDataAsZero") && !options.considerMissingDataAsZero) && !options.legendColors.hasOwnProperty("base"))) {
                    return "subdomain-text subdomain-simpletext-fill";
                }
