            ...
            foreach (var member in type.GetMethods()) {
                ...
                if (isSignal) {
                    ...
                    if (parameters.Length > 0 && parameters[parameters.Length - 1].ParameterType == s_cancellationTokenType) {
                        throw new NotSupportedException($"Signal {memberName} does not support cancellation. See https://github.com/tmds/Antd.DBus/issues/15.");
                    }
                    ...
                    var parameters = member.GetParameters();
                    if (parameters.Length > 0 && parameters[parameters.Length - 1].ParameterType == s_cancellationTokenType) {
                        throw new NotSupportedException($"DBus Method {memberName} does not support cancellation. See https://github.com/tmds/Antd.DBus/issues/15.");
                    }
