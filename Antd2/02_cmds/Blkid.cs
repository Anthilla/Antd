...
namespace Antd2.cmds {
    public class Blkid {
        ...
        public static IEnumerable<(string Partition, string Uuid, string Type, string Label, string Partuuid)> Get() {
            var lines = Bash.Execute(blkidCommand)
                .Select(_ => ParseBlkidLine(_));
            return lines;
        }

        private static (string Partition, string Uuid, string Type, string Label, string Partuuid) ParseBlkidLine(string line) {
            ...
            var partuuid = Help.CaptureGroup(details, "PARTUUID=\"([a-zA-Z0-9\\-\\/]+)\"").Trim();
            return (partition, uuid, type, label, partuuid);
        }
    }
}
