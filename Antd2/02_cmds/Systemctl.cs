...
namespace Antd2.cmds {
    ...
    public class Systemctl {
        ...
        public static SystemService[] Get(SystemctlType type = SystemctlType.none) {
            ...
            var result = Bash.Execute($"{systemctlCommand} {args}").ToArray();
        ...
        public static bool DaemonReload() {
            Bash.Do($"{systemctlCommand} {daemonReloadArg}");
            return true;
        }
        ...
        public static IEnumerable<string> Status(string unit) {
            ...
            return Bash.Execute($"{systemctlCommand} {args}");
        }

        public static bool IsActive(string unit) {
            var args = CommonString.Append(isActiveArg, " ", unit);
            var status = Bash.Execute($"{systemctlCommand} {args}");
            if (status == null) {
                return false;
            }
        ...
        public static string[] GetList() {
            ...
            var result = Bash.Execute($"{systemctlCommand} {args}").Where(_ => !_.Contains("device")).ToArray();
